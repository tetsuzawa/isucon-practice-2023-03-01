...
66,60,空気椅子`
	...
	for _, row := range strings.Split(rawCategoryCsv, "\n") {
		id := 0
		parentId := 0
		categoryName := ""
		for i, col := range strings.Split(row, ",") {
			// id
			if i == 0 {
				id, _ = strconv.Atoi(col)
			} else if i == 1 {
				parentId, _ = strconv.Atoi(col)
			} else if i == 2 {
				categoryName = col
			}
		}
...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := dbx.Select(&items,
			...
			"(SELECT * FROM `items` WHERE `status` IN (?, ?) AND (`created_at` < ?) UNION SELECT * FROM `items` WHERE `status` IN (?, ?) AND (`created_at` <= ? AND `id` < ?)) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
